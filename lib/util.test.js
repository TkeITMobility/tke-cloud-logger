...
describe(__filename, function () {
  ...
  describe('#getDefaultStreams', function () {
    it('should return an empty array if TKE_LOG_SILENT is true', function () {
      envGetStub.withArgs('TKE_LOG_SILENT').returns('true');
      expect(mod.getDefaultStreams()).to.have.length(0);
    });

    it('should return an array of 3 streams if there is a mongo connection and TKE_LOG_SILENT is not true',
      function () {
        envGetStub.withArgs('TKE_LOG_SILENT').returns('abc');
        envGetStub.withArgs('FH_MONGODB_CONN_URL').returns(true);
        expect(mod.getDefaultStreams()).to.have.length(3);
      });

    it('should return an array of 2 streams if there is no mongo connection and TKE_LOG_SILENT is not true',
      function () {
        envGetStub.withArgs('TKE_LOG_SILENT').returns('abc');
        envGetStub.withArgs('FH_MONGODB_CONN_URL').returns(false);
        expect(mod.getDefaultStreams()).to.have.length(2);
      });
  });
});
